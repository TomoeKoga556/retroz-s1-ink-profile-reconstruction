"""Documented public mask approximations for user-owned demonstration images.
...
def public_masks(rgb: np.ndarray, *, for_d3: bool = False) -> dict[str, np.ndarray]:
    ...
    fx = np.zeros_like(luma, dtype=np.float32)
    if for_d3:
        result = {"ink": ink, "flat": flat, "background": background, "fx": fx}
        for value in result.values():
            value.setflags(write=False)
        return result
    recipient = np.clip((local_max - luma - 0.02) / 0.18, 0.0, 1.0).astype(np.float32)
    return {"flat": flat, "ink": ink, "ink_recipient": recipient, "fx": fx,
            "background": background, "local_min_luma": local_min, "local_max_luma": local_max}
